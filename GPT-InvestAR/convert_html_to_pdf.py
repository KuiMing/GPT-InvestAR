...
def main(args):
    ...
    symbol_paths = [
        ...
        for folder in glob.glob(
            os.path.join(config_dict["annual_reports_html_save_directory"], "*")
        )
    ...
    for i, symbol_path in enumerate(symbol_paths):
        ...
        for ar_dates_symbol_path in ar_dates_symbol_paths:
            ar_paths = [
                file
                for file in glob.glob(os.path.join(ar_dates_symbol_path, "*"))
                if os.path.isfile(file)
            ]
            # ar_paths should be a list of 1 element only i.e the report
            assert len(ar_paths) == 1
            date = ar_dates_symbol_path.split("/")[-1]
            pdf_save_dir = os.path.join(
                config_dict["annual_reports_pdf_save_directory"], symbol, date
            )
            ...
            if os.path.exists(pdf_save_path):
                ...
                while not os.path.exists(pdf_save_path):
                    os.system(
                        f"xvfb-run --auto-servernum wkhtmltopdf {ar_paths[0]} {pdf_save_path}"
                    )
        print(f"Completed: {i + 1}/{len(symbol_paths)}- {symbol}")
