...
def make_targets(symbol, start_date, end_date, price_data_sp500, config_dict):
    ...
    df = pd.DataFrame()
    curr_report_date = datetime.strptime(ar_dates[-1], '%Y-%m-%d')
    #Start and end dates are offset by 2 days to be conservative and allowing the price to settle.
    curr_start_date = datetime.strptime(ar_dates[-1], '%Y-%m-%d') + timedelta(days=2)
    try:
        curr_end_date_12m = price_data.index[-1]
    except:
        curr_end_date_12m = price_data_sp500.index[-1]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--start_date', type=str, required=True,
                        help='''Example: 2023-01-01''')
    parser.add_argument('--end_date', type=str, required=True,
                        help='''Example: 2023-12-31''')
